 ...
 ** \file
 **
 ** \brief cFE PSP Ping Related Functions
 **
 ** \copyright
 ...
 ** All Rights Reserved.
 **
 ** \par Description: API for the ping feature, which allow users to send ICMP packets
 ** to specific IPs and return not only about success or failure, but also the
 ** structure including some statistics of the packets.
 **
 **
...
static uint16 CFE_PSP_CalculateICMPChecksum(const uint16 *pData, const uint32 uiLength);

static PSP_Ping_Result_t CFE_PSP_GenerateICMPPkt(uint8 *pBuffer, uint16 *pIdent);

static PSP_Ping_Result_t CFE_PSP_Process_Packet(uint8 *pBuffer, int iRecvBytes, struct sockaddr_in *pFrom,
...
static uint16 g_usPingIdentNum = 0;

/** \brief Tracks whether mutex was initialized already */
static bool g_bInitMutex = false;
