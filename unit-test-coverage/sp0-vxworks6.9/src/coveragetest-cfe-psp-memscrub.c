...
extern osal_id_t g_semUpdateMemAddr_id;
extern bool g_bScrubAddrUpdates_flag;

extern void CFE_PSP_MemScrubTask(void);
extern int32 CFE_PSP_MemScrubValidate(CFE_PSP_MemScrubStatus_t *pNewValues);
...
void Test_CFE_PSP_MemScrubInit(void)
{
    ...
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Unable to create semaphore, OSAL ERROR %d\n", OS_SEM_FAILURE);
    ...
    UT_ResetState(0);
    Ut_OS_printf_Setup();
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Already Initialized\n");
    ...
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Unable to enable mem scrub task\n");
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubInit();
    /* Verify results */
...
/*=======================================================================================
** Test_CFE_PSP_MemScrubTask(void) test cases
**=======================================================================================*/
void Test_CFE_PSP_MemScrubTask(void)
{
    ...
    UT_SetDeferredRetcode(UT_KEY(PCS_scrubMemory), 1, OS_SUCCESS);
    UT_SetDeferredRetcode(UT_KEY(PCS_scrubMemory), 1, OS_ERROR);
    /* Execute test */
    ...
    g_bScrubAddrUpdates_flag = true;
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Unable to take semaphore\n");
    /* Execute test */
    CFE_PSP_MemScrubTask();
    /* Verify outputs */
    ...
    g_uiEndOfRam = 99;
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Unable to give semaphore\n");
    /* Execute test */
    CFE_PSP_MemScrubTask();
    /* Verify outputs */
    ...
    CFE_PSP_MemScrubTask();
    /* Verify outputs */
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Could not Delay Task for Timed Mode\n");
...
void Test_CFE_PSP_MemScrubDisable(void)
{
    ...
    Ut_OS_printf_Setup();
    UT_SetDeferredRetcode(UT_KEY(OS_TaskGetIdByName), 1, OS_ERR_NAME_NOT_FOUND);
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Memory scrub task is not running\n");
    ...
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Unable to delete memory scrub task\n");
    g_MemScrub_Status.uiMemScrubTotalPages = 99;
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubDisable();
    /* Verify results */
    ...
    UT_SetDeferredRetcode(UT_KEY(OS_BinSemGive), 1, OS_SEM_FAILURE);
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Unable to release semaphore\n");
    g_MemScrub_Status.uiMemScrubTotalPages = 99;
...
void Test_CFE_PSP_MemScrubEnable(void)
{
    ...
    /* Set additional inputs */
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Active Memory Scrubbing task is already running\n");
    UT_SetDeferredRetcode(UT_KEY(OS_TaskGetIdByName), 1, OS_SUCCESS);
    /* Execute test */
    ...
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Mem Scrub has not been initialized\n");
    ...
    Ut_OS_printf_Setup();
    g_semUpdateMemAddr_id = OS_ObjectIdFromInteger(99);
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Error creating memory scrub task\n");
    UT_SetDeferredRetcode(UT_KEY(OS_TaskGetIdByName), 1, OS_ERR_NAME_NOT_FOUND);
    UT_SetDeferredRetcode(UT_KEY(OS_TaskCreate), 1, OS_ERROR);
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubEnable();
...
void Test_CFE_PSP_MemScrubValidate(void)
{
    ...
    newMemScrubConfig.uiBlockSize_Pages = 10;
    ...
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Start address after end address\n");
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubValidate(&newMemScrubConfig);
    /* Verify outputs */
    ...
    g_uiEndOfRam = 0;
    tempMemTop = 409600;
    UT_SetDataBuffer(UT_KEY(PCS_sysPhysMemTop), &tempMemTop, sizeof(tempMemTop), true);
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubValidate(&newMemScrubConfig);
    /* Verify outputs */
    ...
    newMemScrubConfig.uiMemScrubStartAddr = 100;
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Invalid new end address\n");
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubValidate(&newMemScrubConfig);
    /* Verify outputs */
    ...
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Priority is outside range\n");
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubValidate(&newMemScrubConfig);
    /* Verify outputs */
    ...
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Incorrect block size\n");
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubValidate(&newMemScrubConfig);
    /* Verify outputs */
...
void Test_CFE_PSP_MemScrubSet(void)
{
    ...
    snprintf(cMsg, sizeof(cMsg), "PSP MEM SCRUB: Priority changed to `%u`\n", newMemScrubConfig.opMemScrubTaskPriority);
    OS_BinSemCreate(&g_semUpdateMemAddr_id, CFE_PSP_MEMSCRUB_BSEM_NAME, OS_SEM_EMPTY, 0);
    UT_SetDeferredRetcode(UT_KEY(OS_BinSemTake), 1, OS_SUCCESS);
    UT_SetDeferredRetcode(UT_KEY(OS_BinSemGive), 1, OS_SUCCESS);
    /* Execute test */
    ...
    UT_SetDeferredRetcode(UT_KEY(OS_BinSemTake), 1, OS_SUCCESS);
    UT_SetDeferredRetcode(UT_KEY(OS_BinSemGive), 1, OS_SUCCESS);
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "New configuration did not pass validation\n");
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubSet(&newMemScrubConfig);
    /* Verify outputs */
    ...
    Ut_OS_printf_Setup();
    g_semUpdateMemAddr_id = OS_OBJECT_ID_UNDEFINED;
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Binary semaphore not created\n");
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubSet(&newMemScrubConfig);
    /* Verify results */
    ...
    OS_BinSemCreate(&g_semUpdateMemAddr_id, CFE_PSP_MEMSCRUB_BSEM_NAME, OS_SEM_EMPTY, 0);
    UT_SetDeferredRetcode(UT_KEY(OS_BinSemTake), 1, OS_SEM_FAILURE);
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Unable to take binary semaphore\n");
    /* Execute test */
    iReturnCode = CFE_PSP_MemScrubSet(&newMemScrubConfig);
    /* Verify results */
...
void Test_CFE_PSP_MemScrubDelete(void)
{
    ...
    UT_SetDeferredRetcode(UT_KEY(OS_TaskGetIdByName), 1, OS_SUCCESS);
    UT_SetDeferredRetcode(UT_KEY(OS_BinSemTake), 1, OS_SEM_FAILURE);
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Unable to disable mem scrub task\n");
    /* Execute test */
    ...
    snprintf(cMsg, sizeof(cMsg), MEMSCRUB_PRINT_SCOPE "Unable to delete semaphore\n");
    OS_TaskCreate(&g_MemScrub_Status.uiMemScrubTaskId, 
                    CFE_PSP_MEMSCRUB_TASK_NAME, 
                    CFE_PSP_MemScrubTask,
                    OSAL_TASK_STACK_ALLOCATE, 
...
/*=======================================================================================
** Test_CFE_PSP_MemScrubTrigger(void) test cases
**=======================================================================================*/
