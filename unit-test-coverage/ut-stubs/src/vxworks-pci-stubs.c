...
PCS_STATUS PCS_pciFindDevice(int vendorId, int deviceId, int index, int *pBusNo, int *pDeviceNo, int *pFuncNo)
{
    ...
    Status = UT_DEFAULT_IMPL(PCS_pciFindDevice);
...
PCS_STATUS PCS_pciConfigInWord(int busNo, int deviceNo, int funcNo, int offset, uint16 *pData)
{
    ...
    iRetCode = UT_DEFAULT_IMPL(PCS_pciConfigInWord);

    UT_Stub_CopyToLocal(UT_KEY(PCS_pciConfigInWord), (uint16 *)pData, sizeof(*pData));
