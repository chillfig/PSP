...
int PCS_sysEnableFpgaWdt(bool globalReset)
{
    ...
    iStatus = UT_DEFAULT_IMPL(PCS_sysEnableFpgaWdt);
...
int PCS_sysDisableFpgaWdt(void)
{
    ...
    iStatus = UT_DEFAULT_IMPL(PCS_sysDisableFpgaWdt);
...
int PCS_sysSetFpgaWdt(float secs)
{
    ...
    iStatus = UT_DEFAULT_IMPL(PCS_sysSetFpgaWdt);
...
{
    UT_DEFAULT_IMPL(PCS_sysPulseFpgaWdt);
}

bool PCS_isCpciSysController(void)
{
    return UT_DEFAULT_IMPL(PCS_isCpciSysController);
}

PCS_SP0_BOARD_GENERATION_TYPE PCS_sysGetBoardGeneration(bool talkative)
{
    return UT_DEFAULT_IMPL(PCS_sysGetBoardGeneration);
}