...
extern uint16 CFE_PSP_CalculateICMPChecksum(const uint16 *pData, const uint32 uiLength);
extern PSP_Ping_Result_t CFE_PSP_GenerateICMPPkt(uint8 *pBuffer, uint16 *pIdent);
extern void ping_Init(uint32 uiPspModuleId);
extern PSP_Ping_Result_t CFE_PSP_Process_Packet(uint8 *pBuffer, int iRecvBytes, struct sockaddr_in *pFrom,
                                                const uint16 usIdent, PSP_Ping_Stats_t *pStatPacket);
extern uint16 g_usPingIdentNum;
extern bool g_bInitMutex;

/*=======================================================================================
** PSP Ping test cases
**=======================================================================================*/

void Test_CFE_PSP_CalculateICMPChecksum(void)
{
    /* Initialize Variable */
    ...
    {
        usData[ucIdx] = ucIdx * 0x10;
    }
    /* Test setups */
    UT_ResetState(0);
    uiExpectedCRCValue = ~(0x3561);
    /* Execute test */
    uiCRCValue = CFE_PSP_CalculateICMPChecksum(usData, sizeof(usData));
    /* Verify outputs */
    ...
    /* Ensure test works on both types of endian systems */
    usData[99] = 0x0606;
    /* Test setups */
    UT_ResetState(0);
    uiExpectedCRCValue = ~(0x2F37);
    /* Execute test */
    uiCRCValue = CFE_PSP_CalculateICMPChecksum(usData, sizeof(usData) - 1);
    /* Verify outputs */
...
void Test_CFE_PSP_GenerateICMPPkt(void)
{
    /* Initialize Variable */
    char cStrTestDescription[256] = {0};
    ...
    UT_SetDefaultReturnValue(UT_KEY(CFE_PSP_GetOSTime), PCS_OK);
    myTime.Seconds = 1;
    myTime.Subseconds = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_PSP_GetOSTime), &myTime, sizeof(myTime), true);
    UT_SetDefaultReturnValue(UT_KEY(PCS_CFE_TIME_Micro2SubSecs), 1000);
    ...
    UtAssert_True(icmpHdr.usChecksum == 64516, cStrTestDescription);
    ...
    Ut_OS_printf_Setup();
    memset(outIcmpPkt, 0, sizeof(outIcmpPkt));
    UT_SetDefaultReturnValue(UT_KEY(CFE_PSP_GetOSTime), PCS_ERROR);
    UT_SetDeferredRetcode(UT_KEY(OS_MutSemTake), 1, OS_SUCCESS);
    UT_SetDeferredRetcode(UT_KEY(OS_MutSemGive), 1, OS_SUCCESS);
...
void Test_CFE_PSP_Process_Packet(void)
{
     /* Initialize Variable */
    char cStrTestDescription[256] = {0};
    ...
    pIpReplyHdr = (struct ip *)inIcmpPkt;
    pIpReplyHdr->ip_hl = IP_HEADER_LEN >> 2;

    /* Pointers to modify the icmp packet info */
    pIcmpHdr = (struct icmphdr *)((char *)inIcmpPkt + IP_HEADER_LEN);
...
/* ----- Test case #5 - Nominal ----- */
    ...
    memset(inIcmpPkt, 0, ICMP_RTN_PACKET_SIZE);
    UT_SetDefaultReturnValue(UT_KEY(PCS_clock_gettime), PCS_OK);
    UT_SetDataBuffer(UT_KEY(PCS_clock_gettime), &getTimeReturnStruct, sizeof(getTimeReturnStruct), false);
    UT_SetDefaultReturnValue(UT_KEY(PCS_CFE_TIME_Micro2SubSecs), 1000);
    UT_SetDataBuffer(UT_KEY(PCS_CFE_TIME_Subtract), &ExpecTime, sizeof(ExpecTime), false);
    ...
    pMyTime = (CFE_TIME_SysTime_t *)((char *)inIcmpPkt + IP_HEADER_LEN + ICMP_HEADER_SIZE);
    pMyTime->Seconds = 0;
    pMyTime->Subseconds = 0;
    UT_SetDataBuffer(UT_KEY(PCS_inet_ntoa), pAddr, sizeof(pAddr), true);
...
void Test_CFE_PSP_GetPacketTypeName(void)
{
    /* Initialize Variable */
    char cStrTestDescription[256] = {0};
    char pBuffer[20] = {0};
...
void Test_CFE_PSP_SinglePing(void)
{
     /* Initialize Variable */
    char cStrTestDescription[256] = {0};
    char cStrOSprintfOutput[MAX_OS_PRINTF_MESSAGES] = {0};
    ...
    UT_SetDataBuffer(UT_KEY(PCS_inet_addr), &destaddr, sizeof(destaddr), false);
    ...
    UT_SetDataBuffer(UT_KEY(PCS_inet_addr), &destaddr, sizeof(destaddr), false);
    UT_SetDeferredRetcode(UT_KEY(PCS_socket), 1, -1);
    ...
    UT_SetDeferredRetcode(UT_KEY(PCS_sendto), 1, -1);
    ...
    UT_SetDeferredRetcode(UT_KEY(OS_MutSemTake), 1, OS_SUCCESS);
    UT_SetDeferredRetcode(UT_KEY(OS_MutSemGive), 1, OS_SUCCESS);
    UT_SetDeferredRetcode(UT_KEY(PCS_sendto), 1, 85);
    ...
    UT_SetDeferredRetcode(UT_KEY(OS_MutSemTake), 1, OS_SUCCESS);
    UT_SetDeferredRetcode(UT_KEY(OS_MutSemGive), 1, OS_SUCCESS);
    UT_SetDeferredRetcode(UT_KEY(PCS_sendto), 1, 64);
    UT_SetDeferredRetcode(UT_KEY(PCS_select), 1, -1);
    ...
    UT_SetDeferredRetcode(UT_KEY(OS_MutSemGive), 1, OS_SUCCESS);
    UT_SetDeferredRetcode(UT_KEY(PCS_sendto), 1, 64);
    UT_SetDeferredRetcode(UT_KEY(PCS_select), 5000, 0);
    ...
    UT_SetDeferredRetcode(UT_KEY(PCS_sendto), 1, 64);
    UT_SetDeferredRetcode(UT_KEY(PCS_select), 1, 1);
    UT_SetDeferredRetcode(UT_KEY(PCS_recvfrom), 1, -1);
    ...
    UT_SetDeferredRetcode(UT_KEY(PCS_recvfrom), 1, 1);
    UT_SetDeferredRetcode(UT_KEY(PCS_select), 1, 1);
    UT_SetDeferredRetcode(UT_KEY(PCS_recvfrom), 1, 100);
    UT_SetDeferredRetcode(UT_KEY(PCS_clock_gettime), 1, PCS_ERROR);
    UT_SetDeferredRetcode(UT_KEY(PCS_select), 5000, 0);
    ...
    UT_SetDeferredRetcode(UT_KEY(PCS_sendto), 1, 64);
    UT_SetDeferredRetcode(UT_KEY(PCS_select), 1, 1);
    UT_SetDeferredRetcode(UT_KEY(PCS_recvfrom), 1, 84);
    UT_SetDataBuffer(UT_KEY(PCS_recvfrom), (void *)inIcmpPkt, sizeof(inIcmpPkt), false);
    UT_SetDefaultReturnValue(UT_KEY(PCS_clock_gettime), PCS_OK);
    UT_SetDataBuffer(UT_KEY(PCS_clock_gettime), &getTimeReturnStruct, sizeof(getTimeReturnStruct), false);
    UT_SetDataBuffer(UT_KEY(PCS_CFE_TIME_Subtract), &CheckTime, sizeof(CheckTime), false);
...
{
    ADD_TEST(Test_CFE_PSP_CalculateICMPChecksum);
    ADD_TEST(Test_CFE_PSP_GenerateICMPPkt);
    ADD_TEST(Test_CFE_Ping_Init);
    ADD_TEST(Test_CFE_PSP_Process_Packet);
    ADD_TEST(Test_CFE_PSP_GetPacketTypeName);
    ADD_TEST(Test_CFE_PSP_SinglePing);
}
